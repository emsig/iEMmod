SUBROUTINE Pupminderiv (Pup, dPup, Rp, Rm, dRp, dRm, Gam, dGam, nk, cor, nz, z, zsrc, zrcvlay, zsrclay, nlayers, above);
! Calculate Pup-
!
! Calling arguments:
...
COMPLEX,INTENT(INOUT) :: Pup(nk), Rp(nk,nz), Rm(nk,nz), dPup(nk,nz), dRp(nk,nz,nz), dRm(nk,nz,nz)
        ...
        do iz=1,nz
            if (iz == zsrclayfort) then
                ...
                elseif (zsrclayfort == 1) then
                    ...
                    term10 = -1.0*dp*exp(-1.0*Gam(ikx,iz)*dp)*dGam(ikx,iz);
                    term11 = -1.0*(ds+dm)*exp(-1.0*Gam(ikx,iz)*(ds+dm))*dGam(ikx,iz);
                    ...
                    term10 = -1.0*dp*exp(-1.0*Gam(ikx,iz)*dp)*dGam(ikx,iz);
                    term11 = -1.0*(ds+dm)*exp(-1.0*Gam(ikx,iz)*(ds+dm))*dGam(ikx,iz);
                    term12 = (term6*dRm(ikx,iz,iz))+(term7*dRp(ikx,iz,iz))&
                        +(term8*term9);
                    dPup(ikx,iz) = (term1*dRp(ikx,iz,iz))+(term3*dRm(ikx,iz,iz))&
                        +(term2*term12)+(term4*term10)+(term5*term11);
                endif
            else
...
elseif (above == 1) then ! receivers above the source layer
    ...
    do ikx=1,nk
        if (zsrclayfort == nz) then ! If the source is in the lower most layer
            Ms = 1.0;
            Pup(ikx) = (1+Rm(ikx,nlayers+1))*(-1.0*exp(-1.0*Gam(ikx,zsrclayfort)*dm));
        else
