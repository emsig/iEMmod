SUBROUTINE dgridit_xx_lin (Ptot,xtotrad1,xtotrad2,dx,nxh,dy,nyh,xpos,nlogx,zrcv,zsrc,&
    etaH,etaV,zetaH,zetaV,gamA,gamB,above,xdirect,df)
    ...
    do ix=1,nxh
        ...
        if (df == 1) then
            if (above==0 .AND. xdirect==1) then
                CALL dgin11(dgin,zrcv,zsrc,etaH,etaV,zetaH,zetaV,gamA,gamB,rad,x,1);
...
END SUBROUTINE dgridit_xx_lin
      
SUBROUTINE dgridit_xy_lin (Ptot,xtotrad,dx,nxh,dy,nyh,xpos,nlogx,zrcv,zsrc,etaH,etaV,zetaH,zetaV,gamA,gamB,above&
        ,xdirect,df)
! Compute the data of one quadrant of the grid based on the radial data using linear interpolation
...
do iy=1,nyh
    ...
    do ix=1,nxh
        ...
        if (df == 1) then
            if (above==0 .AND. xdirect==1) then
                CALL dgin12(gin,zrcv,zsrc,etaH,etaV,zetaH,zetaV,gamA,gamB,rad,x,y,1);
                Ptot(ix,iy) = Ptot(ix,iy) + gin;
            endif
        endif
    end do
end do
END SUBROUTINE dgridit_xy_lin

SUBROUTINE dgridit_xz_lin (Ptot,xtotrad1,dx,nxh,dy,nyh,xpos,nlogx,zrcv,zsrc,etaH,etaV,zetaH,zetaV&
    ,gamA,above,xdirect,df)
! Compute the data of one quadrant of the grid based on the radial data using linear interpolation
...
do iy=1,nyh
    ...
    do ix=1,nxh
        ...
        if (df == 1) then
            if (above==0 .AND. xdirect==1) then
                CALL dgin13(gin,zrcv,zsrc,etaH,etaV,zetaH,zetaV,gamA,rad,x,1);
                Ptot(ix,iy) = Ptot(ix,iy) - gin;
            endif
        endif
    end do
end do
END SUBROUTINE dgridit_xz_lin
