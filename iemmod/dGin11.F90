SUBROUTINE dgin11 (dGin, zrcv, zsrc, etaH, etaV, zetaH, zetaV, gamA, gamB, xpos, x, nlogx)
! Calculate the gradient of the direct field in the space domain of component 11
!
! Calling arguments:
...
COMPLEX,INTENT(INOUT) :: dGin(nlogx)
...
do ix=1,nlogx
    ...
    dGAds = (dGdg*dgds);
    dt1det = ((3.0*x(ix)*x(ix)/(Rbig**2.0)-1.0)*(-1.0/((etaH*Rbig)**2.0)&
        &-sqrt(gamA)/((etaH**2.0)*Rbig)))*G;
    dt1dg = ((3.0*x(ix)*x(ix)/(Rbig**2.0)-1.0)*(+1.0/(etaH*Rbig)))*G;
    dt1dGA = ((3.0*x(ix)*x(ix)/(Rbig**2.0)-1.0)*(1.0/(etaV*Rbig**2.0)&
        &+sqrt(gamA)/(etaV*Rbig))+zetaH*x(ix)*x(ix)/(Rbig**2.0));
