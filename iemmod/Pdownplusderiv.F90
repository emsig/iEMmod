SUBROUTINE Pdownplusderiv (Pdown, dPdown, Rp, Rm, dRp, dRm, Gam, dGam, nk, cor, nz, z, zsrc, zrcvlay, zsrclay, nlayers, above);
! Calculate Pdown+
!
! Calling arguments:
...
COMPLEX,INTENT(INOUT) :: Pdown(nk), dPdown(nk,nz), Rp(nk,nz), Rm(nk,nz), dRp(nk,nz,nz)
...
if (above == 0) then ! receivers in the source layer
    do ikx=1,nk
        ...
        do iz=1,nz
            if (zsrclayfort == 1 .OR. zsrclayfort == nz) then
                Ms = 1.0;
            else
                Ms = 1.0-Rm(ikx,zsrclayfort)*Rp(ikx,zsrclayfort)*exp(-2.0*Gam(ikx,zsrclayfort)*ds);
            endif
            ...
            if (iz == zsrclayfort) then
                ...
                elseif (zsrclayfort == nz) then
                    ...
                    term2  = ((-1.0*Rm(ikx,zsrclayfort))/(Ms*Ms))*(exp(-1.0*Gam(ikx,zsrclayfort)*dm));
                    term3  = ((1.0*Rm(ikx,zsrclayfort))/Ms)*exp(-1.0*Gam(ikx,zsrclayfort)*(ds+dp));
                    term4  = Rm(ikx,zsrclayfort)/Ms;
                    term7  = -Rm(ikx,zsrclayfort)*exp(-2.0*Gam(ikx,zsrclayfort)*ds);
                    term9  = -2.0*ds*exp(-2.0*Gam(ikx,zsrclayfort)*ds)*dGam(ikx,zsrclayfort);
                    term10 = -1.0*dm*exp(-1.0*Gam(ikx,zsrclayfort)*dm)*dGam(ikx,zsrclayfort);
                    term11 = -1.0*(ds+dp)*exp(-1.0*Gam(ikx,zsrclayfort)*(ds+dp))*dGam(ikx,zsrclayfort);
                    dPdown(ikx,iz) = (term1*dRm(ikx,zsrclayfort,zsrclayfort))+(term4*term10);
                else
                    term1  = (1.0/Ms)*(exp(-1.0*Gam(ikx,zsrclayfort)*dm)&
                        +Rp(ikx,zsrclayfort)*exp(-1.0*Gam(ikx,zsrclayfort)*(ds+dp)));
                    term2  = ((-1.0*Rm(ikx,zsrclayfort))/(Ms*Ms))*(exp(-1.0*Gam(ikx,zsrclayfort)*dm)&
                    ...
                    term10 = -1.0*dm*exp(-1.0*Gam(ikx,zsrclayfort)*dm)*dGam(ikx,zsrclayfort);
                    term11 = -1.0*(ds+dp)*exp(-1.0*Gam(ikx,zsrclayfort)*(ds+dp))*dGam(ikx,zsrclayfort);
                    term12 = (term7*dRp(ikx,zsrclayfort,zsrclayfort))+&
                        (term6*dRm(ikx,zsrclayfort,zsrclayfort))+(term8*term9);
                    dPdown(ikx,iz) = (term1*dRm(ikx,zsrclayfort,zsrclayfort))&
                        +(term3*dRp(ikx,zsrclayfort,zsrclayfort))+(term2*term12)&
