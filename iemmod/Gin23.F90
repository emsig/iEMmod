SUBROUTINE gin23 (Gin, zrcv, zsrc, etaH, etaV, zetaH, zetaV, gamA, xpos, y, nlogx);
! Calculate the direct field in the space domain of component 23
!
! Calling arguments:
...
do ix=1,nlogx
    ...
    Gin(ix) = -1.0*y(ix)*etaH/etaV*(zsrc-zrcv)*(gamA/(Rbig**2.0)+3.0*sqrt(gamA)/(Rbig**3.0)+3.0/(Rbig**4))*G/etaV;
end do

END SUBROUTINE gin23
