SUBROUTINE gin31 (Gin, zrcv, zsrc, etaH, etaV, zetaH, zetaV, gamA, xpos, x, nlogx);
! Calculate the direct field in the space domain of component 31
!
! Calling arguments:
...
do ix=1,nlogx
    ...
    Gin(ix) = x(ix)*etaH/etaV*(zrcv-zsrc)*(gamA/(Rbig**2.0)+3.0*sqrt(gamA)/(Rbig**3.0)+3.0/(Rbig**4))*G/etaV;
end do

END SUBROUTINE gin31
