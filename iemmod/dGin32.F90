SUBROUTINE dgin32 (Gin, zrcv, zsrc, etaH, etaV, zetaH, zetaV, gamA, xpos, y, nlogx);
! Calculate the gradient of the direct field in the space domain of component 32
!
! Calling arguments:
...
do ix=1,nlogx
    ...
    dTdGA = y(ix)*(zrcv-zsrc)*(gamA/(Rbig**2.0)+3.0*sqrt(gamA)/(Rbig**3.0)+3.0/(Rbig**4.0))/etaV;
    dTdg = y(ix)*(zrcv-zsrc)*(2.0*sqrt(gamA)/(Rbig**2.0)+3.0/(Rbig**3.0))*G/etaV;
    dTdet = -1.0*y(ix)*(zrcv-zsrc)*(gamA/(Rbig**2.0)+3.0*sqrt(gamA)/(Rbig**3.0)+3.0/(Rbig**4.0))*G/(etaV**2.0);

    Gin(ix) = (dTdGA*dGAds) + (dTdg*dgds) + dTdet;
end do

END SUBROUTINE dgin32
